...
class PointProcess(ConditionalIntensityFunction, ParamSetter, GlobalSeed):
    def __init__(self, CIF = None, params=None, config_file=None, seed=None):
        ...
        self.frequency_domain = create_frequency_domain(self.params['frequencies'], self.pp_PSD)
    ...
    def _extract_lambda(self):
       ...
       cif_timedomain = self.cif.time_domain.get_intensity_realization().squeeze().copy()
       ...
       if np.all(cif_timedomain == cif_timedomain[0]):
           ...
           raise ValueError("Not all values in cif_timedomain are the same. Cannot extract a single lambda value.")
    ...
    def _simulate(self):
       ...
       if self._process_type == "cox":
           ...
           self.time_domain = create_time_domain(self.cif._time_axis, intensity_realization=cif_timedomain, events = spikes)
