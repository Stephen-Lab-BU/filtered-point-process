...
class Filter(ParamSetter, GlobalSeed):
    ...
    def __init__(self, filter_1_model_1=None, filter_2_model_1=None, filter_1_model_2=None, filter_2_model_2=None, model_1=None, model_2=None, filter_1_config_file=None, filter_2_config_file=None):
        ...
        self._validate_filter(filter_1_model_1, filter_2_model_1, filter_1_model_2, filter_2_model_2)

        # Global check for configuration files
        if filter_1_config_file or filter_2_config_file:
            raise ValueError("Configuration files are not allowed. Please use the default parameters.")
        ...
        if model_1 is not None:
            ...
            self.pp_1 = None
            raise ValueError("Model_1 was left as None, please specify a model you want to pass to the filters.")
        ...
        if self.filter_1_model_1:
            self.filter_1_instance_model_1 = self.initialize_filter(self.filter_1_model_1, self.pp_1, self.filter_params)
        if self.filter_2_model_1:
            self.filter_2_instance_model_1 = self.initialize_filter(self.filter_2_model_1, self.pp_1, self.filter_params)
        if self.pp_2 is not None:
            if self.filter_1_model_2:
                self.filter_1_instance_model_2 = self.initialize_filter(self.filter_1_model_2, self.pp_2, self.filter_params)
            if self.filter_2_model_2:
                self.filter_2_instance_model_2 = self.initialize_filter(self.filter_2_model_2, self.pp_2, self.filter_params)

    def _validate_filter(self, filter_1_model_1, filter_2_model_1, filter_1_model_2, filter_2_model_2):
        """Validate filter types."""
        filters = [filter_1_model_1, filter_2_model_1, filter_1_model_2, filter_2_model_2]
    ...
    def _validate_config_filter(self, filter_params, valid_types):
        ...
        if 'method' in filter_params and filter_params['method'] not in valid_types:
            raise ValueError(f"Unsupported filter method in config, please provide one of the following {valid_types}.")
    ...
    def initialize_filter(self, filter_type, point_process, filter_params):
        ...
        if filter_type == 'AMPA':
            ...
            return FastAPFilter(point_process=point_process, filter_params=filter_params)
        elif filter_type == 'Slow_AP':
            return SlowAPFilter(point_process=point_process, filter_params=filter_params)
        elif filter_type == '1/f':
            return LeakyIntegratorFilter(point_process=point_process, filter_params=filter_params)
        else:
...
class FilterBase:
    ...
    def __init__(self, point_process, tau_rise=None, tau_decay=None, filter_params=None, config_file=None, seed=None):
    ...
    def compute_psc(self):
        ...
        self.filter_params['filter_1_time_vector'] = np.arange(0, 0.040, 1/self.pp.params['fs'])
        self._psc_t = np.exp(-self.filter_params['filter_1_time_vector'] / self.filter_params['tau_decay']) - np.exp(
...
class AMPAFilter(FilterBase):
    ...
    def __init__(self, point_process, filter_params=None, tau_rise=None, tau_decay=None):
        """Initialize filter with given point process and default time constants for AMPA."""
        tau_rise = tau_rise if tau_rise is not None else filter_params.get('tau_rise', 0.4 / 1000)
        tau_decay = tau_decay if tau_decay is not None else filter_params.get('tau_decay', 2 / 1000)
...
class GABAFilter(FilterBase):
    ...
    def __init__(self, point_process, filter_params=None, tau_rise=None, tau_decay=None):
        ...
        tau_rise = tau_rise if tau_rise is not None else filter_params.get('tau_rise', 0.4 / 1000)
        tau_decay = tau_decay if tau_decay is not None else filter_params.get('tau_decay', 10 / 1000)
        super().__init__(point_process, tau_rise, tau_decay, filter_params)
        self.compute_psc()
...
class LeakyIntegratorFilter:
    ...
    def __init__(self, point_process, A=1/0.1, filter_params=None, config_file=None, seed=None):
        """Initialize filter with given point process and time constants."""
    ...
    def compute_li(self):
        ...
        self._li_t = np.exp(-self.filter_params['filter_2_time_vector'] * self.filter_params['A'])
...
class FastAPFilter:
    ...
    def compute_kernel(self):
        ...
        t = np.linspace(-self.pp.params['T']/2, self.pp.params['T']/2, int(self.pp.params['fs']*self.pp.params['T']))
        f = self.pp.params['frequencies']

        self._kernel_t = self.k * np.exp(-((t - self.t0)**2) / (2 * self.sigma**2)) * np.cos(2 * np.pi * self.f0 * (t - self.t0) + self.theta)
        self._kernel_f = self.k * np.exp(-2 * np.pi**2 * self.sigma**2 * (f - self.f0)**2) * np.exp(1j * (self.theta - 2 * np.pi * f * self.t0)) + \
    ...
    def kernel_time_axis(self):
        """Return the time axis."""
        return np.linspace(-self.pp.params['T']/2, self.pp.params['T']/2, int(self.pp.params['fs']*self.pp.params['T']))
