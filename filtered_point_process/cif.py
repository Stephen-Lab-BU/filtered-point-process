...
from .helpers import TimeDomain, FrequencyDomain, create_frequency_domain, create_time_domain
...
class ConditionalIntensityFunction(ParamSetter, GlobalSeed):
    def __init__(self, params=None, seed=None, config_file=None):
        ...
        self._time_axis = np.linspace(0, self.params['T'], N, endpoint=False)  # Time vector
        ...
        self.frequency_domain = create_frequency_domain(self.params['frequencies'], self.PSD)
...
######################### CORE FUNCTIONALITY #########################
    ...
    def _simulate(self):
       ...
       self.time_domain = create_time_domain(self._time_axis, intensity_realization=intensity)
   ...
   ######################### HOMOGENEOUS POISSON PROCESS #########################
    ...
    def _simulate_homog_pois(self):
       ...
       return np.real(self._rate_function(
            homogenous_pois_cif_S, self.params['fs']
       ))
    ...
    def _rate_function(self, S, fs):
       ...
       rate_function_realizations = np.tile(self.params["lambda_0"] * np.ones_like(self._time_axis),
                                           (self.params["Nsims"], 1))
   ...
   ######################### GAUSSIAN PROCESS CIF #########################
    ...
    def _compute_cox_cif_S(self):
       ...
       cox_cif_S = self.params['peak_height'] * np.exp(-(self.params['frequencies'].squeeze() - self.params['center_frequency']) ** 2 / (2 * self.params['peak_width'] ** 2))
   ...
   ######################### AR(1) CIF #########################
    ...
    def _compute_ar1_cif_S(self):
       ...
       spectrum = (self.params["white_noise_variance"] ** 2) / (1 - 2 * self.params["phi_1"] * np.cos(omega) + self.params["phi_1"] ** 2)
    ...
    def _compute_ar1_time(self, n_points, n_sims, c):
       ...
       for sim in range(n_sims):
           ...
           for t in range(1, n_points):
               time_series[t] = c + self.params["phi_1"] * time_series[t - 1] + white_noise[t]
    ...
    def _simulate_ar1(self):
       ...
       var_Xt = self.params["white_noise_variance"]**2 / (1 - self.params["phi_1"]**2)
